...
export default class FBXLoader {
  ...
  public load = (url: string, onLoad, onProgress, onError) => {
    ...
    loader.load(url, (buffer) => {
      ...
      try {
        const scene = this.parse(buffer, resourceDirectory);
  ...
  private parse = (FBXBuffer, resourceDirectory) => {
    let FBXTree;
    ...
    const textures = parseTextures(
      FBXTree, new THREE.TextureLoader(this.manager).setPath(resourceDirectory), images, connections,
    );
...
Object.assign(TextParser.prototype, {
  ...
  parseNodeBegin: (_, property) => {
    ...
    const node: any = { name: nodeName, properties: {}, subNodes: {} };
  ...
  parseNodeProperty: (line, property, contentLine) => {
    ...
    if (propName === 'C') {
      ...
      const from = parseInt(connProps[0], 10);
      const to = parseInt(connProps[1], 10);
    ...
    // Node
    if (propName === 'Node') {
      const id = parseInt(propValue, 10);
  ...
  parseNodeSpecialProperty: (_1, _2, propValue) => {
    ...
    switch (innerPropType1) {
      ...
      case 'bool':
      case 'ULongLong':
        innerPropValue = parseInt(innerPropValue, 10);
        break;
  ...
  parseNode: (reader, version) => {
    ...
    while (endOffset > reader.getOffset()) {
      ...
      if (name === 'Connections' && node.name === 'C') {
        ...
        node.propertyList.forEach((property, i) => {
          array[i - 1] = property;
        });
  ...
  parseProperty: (reader) => {
    ...
    switch (type) {
      ...
      case 'l':
        ...
        const reader2 = new BinaryReader(inflated.buffer);
...
Object.assign(BinaryReader.prototype, {
  ...
  getString: (size) => {
    const data = this.getArrayBuffer(size);
    const s = THREE.LoaderUtils.decodeText(data);
    return s;
  },
});

// Parses FBXTree.Connections which holds parent-child connections between objects
// (e.g. material -> texture, model->geometry)
// and details the connection type
function parseConnections(FBXTree) {
  const connectionMap = new Map();

  if ('Connections' in FBXTree) {
    const rawConnections = FBXTree.Connections.properties.connections;
...
function parseTexture(textureNode, loader, imageMap, connections) {
  const texture = loadTexture(textureNode, loader, imageMap, connections);
...
function parseMaterial(FBXTree, materialNode, textureMap, connections) {
  ...
  const parameters = parseParameters(FBXTree, materialNode.properties, textureMap, ID, connections);
  let material;
...
function parseDeformers(FBXTree, connections) {
  ...
  if ('Deformer' in FBXTree.Objects.subNodes) {
    ...
    DeformerNodes.map((nodeID) => {
      const deformerNode = DeformerNodes[nodeID];

      if (deformerNode.attrType === 'Skin') {
        const relationships = connections.get(parseInt(nodeID, 10));
        const skeleton: any = parseSkeleton(relationships, DeformerNodes);
        skeleton.ID = nodeID;
...
function parseMeshGeometry(FBXTree, relationships, geometryNode, skeletons) {
  ...
  if ('GeometricRotation' in modelNode.properties) {
    const array = modelNode.properties.GeometricRotation.value.map(THREE.Math.degToRad);
    array[3] = 'ZYX';
...
function genGeometry(_1, _2, geometryNode, skeleton, preTransform) {
  const subNodes = geometryNode.subNodes;

  const vertexPositions = subNodes.Vertices.properties.a;
  const vertexIndices = subNodes.PolygonVertexIndex.properties.a;
  ...
  vertexIndices.forEach((vertexIndex, polygonVertexIndex) => {
    ...
    if (vertexIndex < 0) {
      vertexIndex = (vertexIndex * -1) - 1; // equivalent to ( x * -1 ) - 1
    ...
    if (colorInfo) {
      const data = getData(polygonVertexIndex, polygonIndex, vertexIndex, colorInfo);
    ...
    if (normalInfo) {
      const data = getData(polygonVertexIndex, polygonIndex, vertexIndex, normalInfo);
    ...
    if (materialInfo && materialInfo.mappingType !== 'AllSame') {
      materialIndex = getData(polygonVertexIndex, polygonIndex, vertexIndex, materialInfo)[0];
    }
  ...
  const positionAttribute = new THREE.Float32BufferAttribute(vertexBuffer, 3);
...
function getNormals(NormalNode) {
  ...
  const buffer = NormalNode.subNodes.Normals.properties.a;
  let indexBuffer = [];
...
function getUVs(UVNode) {
  ...
  const buffer = UVNode.subNodes.UV.properties.a;
  let indexBuffer = [];
  if (referenceType === 'IndexToDirect') {
...
function getMaterials(MaterialNode) {
  ...
  const materialIndexBuffer = MaterialNode.subNodes.Materials.properties.a;
...
// 	- vertexIndex - Index of vertex inside vertex buffer
// (used because some data refers to old index buffer that we don't use anymore).
...
const GetData = {
  ByPolygonVertex: {
    Direct: (polygonVertexIndex, _1, _2, infoObject) => {
    ...
    IndexToDirect: (polygonVertexIndex, _1, _2, infoObject) => {
  ...
  ByPolygon: {
    Direct: (_1, polygonIndex, _2, infoObject) => {
    ...
    IndexToDirect: (_1, polygonIndex, _2, infoObject) => {
  ...
  ByVertice: {
    Direct: (_1, _2, vertexIndex, infoObject) => {
  ...
  AllSame: {
    IndexToDirect: (_1, _2, _3, infoObject) => {
...
function parseNurbsGeometry(geometryNode) {
  if (THREE.NURBSCurve === undefined) {
    console.error(
      'FBXLoader: The loader relies on THREE.NURBSCurve for any nurbs ' +
      'present in the model. Nurbs will show up as empty geometry.',
    );
    return new THREE.BufferGeometry();
  }

  const order = parseInt(geometryNode.properties.Order, 10);

  if (isNaN(order)) {
    console.error(
      'FBXLoader: Invalid Order %s given for geometry ID: %s', geometryNode.properties.Order, geometryNode.id,
    );
...
function parseScene(FBXTree, connections, skeletons, geometryMap, materialMap) {
  ...
  const modelNodes = FBXTree.Objects.subNodes.Model;
...
function createCamera(FBXTree, relationships) {
  ...
  relationships.children.forEach((child) => {
    const attr = FBXTree.Objects.subNodes.NodeAttribute[child.ID];
    ...
    switch (type) {
      ...
      case 1: // Orthographic
        model = new THREE.OrthographicCamera(
          -width / 2, width / 2, height / 2, - height / 2, nearClippingPlane, farClippingPlane,
        );
        break;
...
function createLight(FBXTree, relationships) {
  ...
  if (lightAttribute === undefined) {
    ...
    switch (type) {
      ...
      default:
        console.warn(
          'FBXLoader: Unknown light type ' + lightAttribute.LightType.value + ', defaulting to a THREE.PointLight.',
        );
...
function createMesh(_, relationships, geometryMap, materialMap) {
  let model;
  let geometry = null;
  let material = null;
  const materials = [];
...
// Parse ambient color in FBXTree.GlobalSettings.properties
// if it's not set to black (default), create an ambient light
...
function setModelTransforms(_, model, modelNode) {
  ...
  if ('RotationOrder' in modelNode.properties) {
    ...
    const value = parseInt(modelNode.properties.RotationOrder.value, 10);
    if (value > 0 && value < 6) {
      ...
      console.warn(
        'FBXLoader: unsupported Euler Order: %s. ' +
        'Currently only XYZ order is supported. Animations and rotations may be incorrect.',
        enums[value],
      );
  ...
  if ('Lcl_Rotation' in modelNode.properties) {
    const rotation = modelNode.properties.Lcl_Rotation.value.map(THREE.Math.degToRad);
    rotation.push('ZYX');
  ...
  if ('PreRotation' in modelNode.properties) {
    const array = modelNode.properties.PreRotation.value.map(THREE.Math.degToRad);
    array[3] = 'ZYX';
...
function bindSkeleton(FBXTree, skeletons, geometryMap, modelMap, connections, sceneGraph) {
  ...
  if ('Pose' in FBXTree.Objects.subNodes) {
    const BindPoseNode = FBXTree.Objects.subNodes.Pose;

    for (const nodeID in BindPoseNode) {
      if (BindPoseNode[nodeID].attrType === 'BindPose') {
        const poseNodes = BindPoseNode[nodeID].subNodes.PoseNode;

        if (Array.isArray(poseNodes)) {
          poseNodes.forEach((node) => {
            const rawMatWrd = new THREE.Matrix4().fromArray(node.subNodes.Matrix.properties.a);
            worldMatrices.set(parseInt(node.properties.Node, 10), rawMatWrd);
          });
        } else {
          const rawMatWrd = new THREE.Matrix4().fromArray(poseNodes.subNodes.Matrix.properties.a);
          worldMatrices.set(parseInt(poseNodes.properties.Node, 10), rawMatWrd);
        }
      }
    }
  }
  ...
  skeletons.map((ID) => {
    ...
    skeleton.bones.forEach((bone, i) => {
      ...
      if (worldMatrices.has(bone.ID)) {
        const mat = worldMatrices.get(bone.ID);
    ...
    const parents = connections.get(parseInt(skeleton.ID, 10)).parents;
...
function parseAnimationCurveNodes(FBXTree) {
  const rawCurveNodes = FBXTree.Objects.subNodes.AnimationCurveNode;
  const curveNodesMap = new Map();

  rawCurveNodes.map((nodeID) => {
...
function parseAnimationLayers(FBXTree, connections, curveNodesMap) {
  ...
  rawLayers.map((nodeID) => {
    const layerCurveNodes = [];
    const connection = connections.get(parseInt(nodeID, 10));
      ...
      children.forEach((child, i) => {
        if (curveNodesMap.has(child.ID)) {
          ...
          if (layerCurveNodes[i] === undefined) {
            ...
            const rawModel = FBXTree.Objects.subNodes.Model[modelID.toString()];
...
function parseAnimStacks(FBXTree, connections, layersMap) {
  ...
  rawStacks.map((nodeID) => {
    const children = connections.get(parseInt(nodeID, 10)).children;

    if (children.length > 1) {
      // it seems like stacks will always be associated with a single layer. But just in case there are files
      // where there are multiple layers per stack, we'll display a warning
      console.warn(
        'FBXLoader: Encountered an animation stack with multiple layers, ' +
        'this is currently not supported. Ignoring subsequent layers.',
      );
...
function generateTracks(rawTracks) {
  ...
  if (rawTracks.T !== undefined) {
    const positionTrack = generateVectorTrack(
      rawTracks.modelName, rawTracks.T.curves, rawTracks.initialPosition, 'position',
    );
  ...
  if (rawTracks.R !== undefined) {
    const rotationTrack = generateRotationTrack(
      rawTracks.modelName, rawTracks.R.curves, rawTracks.initialRotation, rawTracks.preRotations,
    );
